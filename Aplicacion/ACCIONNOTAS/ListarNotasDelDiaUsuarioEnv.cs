...
namespace Aplicacion.ACCIONNOTAS {
    public class ListarNotasDelDiaUsuarioEnv {
        ...
        public class Manejador : IRequestHandler<Ejecuta, List<NOTAS>> {
            ...
            public async Task<List<NOTAS>> Handle (Ejecuta request, CancellationToken cancellationToken) {
                var lista = await _context.NOTAS.Where (x => x.FECGRA.Date == DateTime.Now.Date && x.DIFUSION != "SI" && x.RemitenteUserName == request.UserName).OrderByDescending (x => x.FECGRA).ToListAsync ();
                return lista;
            }
        }
    }

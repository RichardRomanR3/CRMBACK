...
namespace Aplicacion.ACCIONNOTAS {
    public class ListarNotasPorFecha {
        ...
        public class Manejador : IRequestHandler<Ejecuta, List<NOTAS>> {
            ...
            public async Task<List<NOTAS>> Handle (Ejecuta request, CancellationToken cancellationToken) {
                var lista = await _context.NOTAS.Where (x => x.FECGRA.Date == request.FECGRA.Date && x.DIFUSION != "SI" && x.DestinatarioUserName == request.DestinatarioUserName).ToListAsync ();
                return lista;
            }
        }
